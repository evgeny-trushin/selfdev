...
Manages the todo/increment_XXXX_todo_*.md ↔ done lifecycle:
  1. Find the current TODO increment (lowest numbered _todo_ file).
  2. Read it and resolve all referenced principles from how/.
...
class IncrementTracker:
    ...
    def __init__(self, root_dir: Path):
        ...
        self.requirements_dir = root_dir / TODO_DIRNAME
        self.principles_dir = root_dir / HOW_DIRNAME
    ...
    def format_self_inspection_prompt(self, increment_path: Path,
        ...
        lines.append("  2. Run tests:  python -m pytest tests/")
        ...
        lines.append("  7. Run ./todo.sh  (verify & get next increment)")
        lines.append("")
    ...
    def format_increment_prompt(self, increment_path: Path) -> str:
        ...
        lines.append("    - Do NOT auto-advance increments by rerunning todo.sh.")
        ...
        lines.append(f"  2. Run tests:  python -m pytest tests/")
        ...
        lines.append(f"  7. Run ./todo.sh  (verify & get next increment)")
        lines.append("")
    ...
    def format_verification_prompt(self, increment_path: Path) -> str:
        ...
        lines.append("")
        lines.append(f"    mv {TODO_DIRNAME}/{old_name} {TODO_DIRNAME}/{new_name}")
        lines.append("")
        lines.append("  Or equivalently run:")
        lines.append("    ./todo.sh --advance")
        lines.append("")
    ...
    def format_revert_prompt(self, increment_number: int) -> str:
        ...
        if commits:
            ...
            lines.append(f"  4. Run tests:  python -m pytest tests/")
            ...
            lines.append("  3. Run tests:  python -m pytest tests/")
    ...
    def format_redo_prompt(self, increment_number: int) -> str:
        ...
        lines.append(f"  3. Run tests:  python -m pytest tests/")
        ...
        lines.append(f"  8. Run ./todo.sh  (verify & get next increment)")
        lines.append("")
