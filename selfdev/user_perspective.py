...
class UserPerspective(PerspectiveAnalyzer):
    ...
    def _check_requirements(self, fitness_components, prompts):
        ...
        Each generated prompt also references principles.md so that
        implementations adhere to the project's development principles.
        ...
        try:
            req_display = str(requirements_path.relative_to(self.root_dir))
        except ValueError:
            req_display = requirements_path.name

        # Mark files that live at the project root (outside selfdev/)
        selfdev_dir = self.root_dir / "selfdev"
        if requirements_path.parent == self.root_dir and selfdev_dir.is_dir():
            req_display = f"{req_display} (project root, outside selfdev/)"

        # Resolve principles.md (project root first, then selfdev dir)
        principles_path = self.root_dir / "principles.md"
        if not principles_path.exists():
            if PRINCIPLES_FILE.exists() and PRINCIPLES_FILE.parent.parent == self.root_dir:
                principles_path = PRINCIPLES_FILE
            else:
                principles_path = None

        if principles_path and principles_path.exists():
            try:
                princ_display = str(principles_path.relative_to(self.root_dir))
            except ValueError:
                princ_display = principles_path.name
            # Mark files that live at the project root (outside selfdev/)
            if principles_path.parent == self.root_dir and selfdev_dir.is_dir():
                princ_display = f"{princ_display} (project root, outside selfdev/)"
        else:
            princ_display = None
        ...
        if princ_display:
            principles_note = (
                f" Implementation must adhere to principles defined in "
                f"{princ_display}."
            )

        for req in reqs:
            priority = self._classify_priority(req['id'])
            description = req['description'] + principles_note
            criteria = list(
                req['acceptance_criteria'] if req['acceptance_criteria']
                else [req['title']]
            )
            criteria.append(f"Follow development principles from {princ_display}" if princ_display else "Follow project development principles")
            prompts.append(Prompt(
                perspective=Perspective.USER,
                priority=priority,
                title=f"[{req['id']}] {req['title']}",
                description=description,
                file_path=req_display,
