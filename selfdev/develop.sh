...
show_help() {
    ...
    echo "  --force      Force advance even with uncommitted changes"
    ...
    echo "  $(basename "$0") --redo=0001   # Revert and redo increment 0001"
    echo ""
    ...
    echo "  3. Run tests:  python -m pytest selfdev/tests/"
    echo "  4. git add -A && git commit -m \"INCREMENT XXXX: desc\" && git push"
    echo "  5. Run ./develop.sh --advance → verify tests pass & changes committed"
    echo "  6. Run ./develop.sh           → see next increment"
...
main() {
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        ...
        echo "║  4. Commit:  git add -A && git commit -m \"INCREMENT XXXX: desc\"     ║"
        echo "║  5. Advance: ./develop.sh --advance                                  ║"
        echo "║     (will verify tests pass and changes are committed)               ║"
        echo "║  6. Run ./develop.sh  (get next increment)                           ║"
        echo "║                                                                      ║"
        echo "║  RULES:                                                              ║"
        echo "║  • Do NOT auto-advance by rerunning develop.sh prematurely           ║"
        echo "║  • Only mark done after tests pass & changes are committed           ║"
