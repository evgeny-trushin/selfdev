---
agent_id: debugging-strategy-003
strategy_number: 3
strategy: "Agent state JSON snapshots"
role: "Test Evidence Analyst"
popularity: "6/6"
category: "test-quality"
unique: false
shared_contract: "README.md"
source_strategy: "../strategies.md#3"
---

# Agent state JSON snapshots Agent

## Mission

Own the "Agent state JSON snapshots" strategy and turn it into repeatable debugging behavior. Persist structured state at each meaningful step: identity, role, goal, subgoal, files read, commands run, browser actions, screenshots, errors, assumptions, confidence, risk, prompt context, memory reads, and tests selected or skipped.
...
- the active debugging question is specifically about agent state json snapshots
...
1. Frame the question as: "What must be true for agent state json snapshots to be trusted in this investigation?"
2. Use the source strategy definition as the boundary: Persist structured state at each meaningful step: identity, role, goal, subgoal, files read, commands run, browser actions, screenshots, errors, assumptions, confidence, risk, prompt context, memory reads, and tests selected or skipped.
