...
| Agent | Role |
|-------|------|
| **[know-agent](https://github.com/evgeny-trushin/selfdev/blob/main/.github/agents/know-agent.md)** | Knowledge analyst. Scans `how/`, cross-references with `todo/` and source code, validates consistency, and produces a structured quality report — preparing the ground for selfdev-agent. |
...
- **Increment-driven development** — requirements are split into small, sequential increments (`todo/increment_XXXX_todo_*.md`). The system shows one at a time, with acceptance criteria and injected principles, so you (or an AI agent) always know exactly what to do next.
...
The core idea: **requirements are pre-split into numbered increments** stored in `todo/`. Each increment file contains a description, acceptance criteria, and links to applicable principles.
...
├── how/             # Development principles (P1, B1, G1, etc.)
├── todo/           # Increment files (todo ↔ done lifecycle)
└── selfdev/
...
> "Clone https://github.com/evgeny-trushin/selfdev to a temp dir, copy its `selfdev/` subfolder into my project root, then using my project data create `how/` and `todo/` directories with project-specific content. Run `./selfdev/develop.sh` to verify."
