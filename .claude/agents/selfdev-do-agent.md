...
description: |
  End-to-end development agent. Takes a user's feature request, creates a requirement in /todo,
...
Knowledge lives in `how/`. State is tracked in `todo/` (todo→done). System health is in `organism_state.json`.
...
USER REQUEST
    ...
    ▼
┌─────────────────────────────┐
│  PHASE 1 — REQUIREMENT      │  Create requirement file in todo/
...
### 1.1 — Load Knowledge
- Read relevant `how/` files for domain context and coding standards:
...
- Read the last 3–5 `todo/increment_*_done_*.md` files (if any) to learn the format, field conventions, and numbering.
...
- Determine the next increment number by scanning `todo/` for the highest existing number and adding 1.
- Create `todo/increment_NNNN_todo_<slug>.md` following the established format:
  - Header: `# Increment NNNN: <Title>`
  - Metadata: Requirement ID, Status (`TODO`)
  - Sections: Description, Related Principles (linking to `how/` files), Acceptance Criteria
...
- Read the full requirement markdown file from `todo/`.
- Identify every acceptance criterion.
- Read relevant `how/` files referenced in the `## Related Principles` section.
...
```bash
mv todo/increment_NNNN_todo_<slug>.md todo/increment_NNNN_done_<slug>.md
```
...
- **Principles applied:** Which `how/` files guided the implementation.
...
**Principle Context:** [Relevant how/ reference]
...
8. **Principles inform approach, not scope.** `how/` guides HOW, never overrides WHAT.
...
| Item | Convention |
|------|-----------|
| Requirement file | `todo/increment_NNNN_todo_<slug>.md` |
| Done requirement | `todo/increment_NNNN_done_<slug>.md` |
...
| Knowledge base | `how/` (principle files) |
